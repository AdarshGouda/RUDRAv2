/*Create Date: 17-Apr-2021
  ...
  //Revision: AB
  //Rev Date: 26-Apr-2021


  //Intent: Integrate IMU data and publish /IMU, imu_pu was missing added it
            Added calibration sequence for MPU in setup()
            corrected wheel radius.
            Added complementary filter to fuse odometry
...
float integral_, derivative_, prev_error_;    // errors for Kp, Kd and Ki
int min_val_ = -127;                          //Sabertooth ranges form -127 to 127, Zero being stop.
int max_val_ = 127;
...
void setup() {
  ...
  nh.getHardware()->setBaud(115200);         //set baud for ROS serial communication (does not work really)
...
void loop() {
  ...
  { // enter timed loop
    ...
    ax = ax * (double) ACCEL_SCALE * G_TO_ACCEL;    //refer to MPU6050 datasheet
    ay = ay * (double) ACCEL_SCALE * G_TO_ACCEL;
    az = az * (double) ACCEL_SCALE * G_TO_ACCEL;

    gx = gx - gyro_bias_x;                          //substracting the bias calculated during calibration
    gy = gy - gyro_bias_y;
    gz = gz - gyro_bias_z;
    ...
    speed_act_left_avg = (speed_act_left1 + speed_act_left2) / 2.0;      //Simply averaging the calculated speeds for left and right wheels
...
/******************* Kalman Filter Implementation to fuse IMU data with wheel odometry ***********************/
